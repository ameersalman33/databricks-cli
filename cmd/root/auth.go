...
// agentguardHostForProfile returns the value of the optional `agentguard` field
...
func agentguardHostForProfile(ctx context.Context, profileName string) string {
	if profileName == "" {
		return ""
	}
	...
	return s.Key("agentguard").String()
}

// agentguardTransport is a path-routing http.RoundTripper. Requests whose path
// starts with /api/ are rewritten to send their traffic to the AgentGuard
...
// at the real workspace while routing data-plane traffic through AgentGuard,
// without any changes to databricks-sdk-go.
type agentguardTransport struct {
	agentguardHost string // URL like https://agentguard-<id>.cloud.databricksapps.com
	inner          http.RoundTripper
}

func (t *agentguardTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if req.URL != nil && strings.HasPrefix(req.URL.Path, "/api/") {
		if u, err := url.Parse(t.agentguardHost); err == nil && u.Host != "" {
...
func MustWorkspaceClient(cmd *cobra.Command, args []string) error {
	...
	// If the profile defines an `agentguard` field, install a custom
	// http.RoundTripper that path-routes API traffic to the AgentGuard
	...
	if v := agentguardHostForProfile(cmd.Context(), cfg.Profile); v != "" {
		inner := cfg.HTTPTransport
		if inner == nil {
			inner = http.DefaultTransport
		}
		cfg.HTTPTransport = &agentguardTransport{agentguardHost: v, inner: inner}
	}
